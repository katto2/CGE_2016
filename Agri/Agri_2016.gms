...
*operation=1;
SET
$include "..\SAM\set_agri_recursive_2016.txt"
...
ActR(A)$(sum(C,SAM(A,C))>0 and not ghg('process',A) and sum(AgriC,SAM(AgriC,A))>0)..PC(A)*alpha_nres(A)*XC(A)+sum(C$(M(C) and not AgriC(C)),PA(C)*ica(C,A)*XC(A))+PVAE(A)*QVAE(A)+PAG(A)*XAG(A)=g=PC(A)*(1-ta_in(A)-ta_ex(A))*XC(A)+crevI*crevIw(A)*CREV(A);
ActR.m(A)=1;
ActRp(A)$(ghg('process',A) ne 0 and sum(AgriC,SAM(AgriC,A))>0)..PC(A)*alpha_nres(A)*XC(A)+sum(C$(M(C) and not AgriC(C)),PA(C)*ica(C,A)*XC(A))+sum(GC,thetaP(GC,A)*XC(A)*gtax(GC)*cpi)+PVAE(A)*QVAE(A)+PAG(A)*XAG(A)=g=PC(A)*(1-ta_in(A)-ta_ex(A))*XC(A)+crevI*crevIw(A)*CREV(A);
ActRP.m(A)=1;

ActRna(A)$(sum(C,SAM(A,C))>0 and not ghg('process',A) and sum(AgriC,SAM(AgriC,A))=0)..PC(A)*alpha_nres(A)*XC(A)+sum(C$(M(C) and not AgriC(C)),PA(C)*ica(C,A)*XC(A))+PVAE(A)*QVAE(A)=g=PC(A)*(1-ta_in(A)-ta_ex(A))*XC(A)+crevI*crevIw(A)*CREV(A);
ActRna.m(A)=1;
ActRpna(A)$(ghg('process',A) ne 0 and sum(AgriC,SAM(AgriC,A))=0)..PC(A)*alpha_nres(A)*XC(A)+sum(C$(M(C) and not AgriC(C)),PA(C)*ica(C,A)*XC(A))+sum(GC,thetaP(GC,A)*XC(A)*gtax(GC)*cpi)+PVAE(A)*QVAE(A)=g=PC(A)*(1-ta_in(A)-ta_ex(A))*XC(A)+crevI*crevIw(A)*CREV(A);
ActRPna.m(A)=1;
...
VAGPr(A)$(sum(C$AgriC(C),SAM(C,A))>0)..PAG(A)=e=(1/alphaaAG(A))*prod(C$(XAPA(C,A) and AgriC(C)),(PA(C)/deltaC(C,A))**deltaC(C,A));
VAGPr.m(A)=1;
...
XAGD(A)$(sum(AgriC,SAM(AgriC,A))>0)..XAG(A)=e=XC(A);
XAGD.m(A)=1;
...
INTDM(C,A)$((M(C) and not AgriC(C)) and SAM(C,A)$(M(C) and not AgriC(C))>0)..XAP(C,A)=e=ica(C,A)*XC(A);
INTDM.m(C,A)=1;
INTDAG0(C,A)$(AgriC(C) and SAM(C,A)$AgriC(C)>0 and Agri_single(A) eq 0)..XAP(C,A)=e=(1/alphaaAG(A))*XAG(A);
INTDAG0.m(C,A)=1;
INTDAG1(C,A)$(AgriC(C) and SAM(C,A)$AgriC(C)>0 and Agri_single(A) eq 1)..XAP(C,A)=e=deltaC(C,A)*(PAG(A)/PA(C))*XAG(A);
INTDAG1.m(C,A)=1;
...
$include  "..\SAM\parameter\gtaxp_BAU.txt";
$include  "..\SAM\parameter\gtaxp_ctax_1.txt";
...
$include "..\SAM\parameter\lgrow.txt";
$include "..\SAM\parameter\Oldpopg.txt";
$include "..\SAM\parameter\TBg.txt";
