...
*$include "..\SAM\Agri\set_agri_static_link_2016_alt_cons.txt"
$include ".\data\set_agri_static_link_2016_alt_cons.txt"
...
dev_XCREP(A)$(ord(t) gt 1)=100*(XCREP(A,t)/XCREP(A,t-1) - 1);
dev_PAREP(C)$(ord(t) gt 1)=100*(PAREP(C,t)/PAREP(C,t-1) - 1);
*dev_q_outj(j)$(ord(t) gt 1)=100*(q_outj_REP(j,t)/q_outj_REP(j,t-1) - 1);
*dev_p_outj(j)$(ord(t) gt 1)=100*(p_outj_REP(j,t)/p_outj_REP(j,t-1) - 1);

*stop$((smax(A, abs(dev_XCREP(A))) lt (1/10)*1e-2) and (smax(C, abs(dev_PAREP(C))) lt (1/10)*1e-2))  = 1;
stop$((smax(A, abs(dev_XCREP(A))) lt (1/10)*con_crit) and (smax(C, abs(dev_PAREP(C))) lt (1/10)*con_crit))  = 1;
...
Display S_new, C_new, q_outj, p_outj, ptc_j, dev_XCREP, dev_PAREP, counter,con_crit;
