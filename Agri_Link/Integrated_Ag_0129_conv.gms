...
*$include "set_Agri_20160119_static_nonlink.txt"
...
PAREP_dev  deviation of PAREP
XCREP_dev  deviation of XCREP
...
rent0=1;
rent0=0;
ZD0(A)$LinkA(A)=0.0*sum(K,R0(K)*KD0(K,A));
...
*                dev_q_outj(j)    Deviation of q_outj values between subsequent solve,
*                dev_p_outj(j)    Deviation of p_outj values between subsequent solve;
...
**************************************************************************************
*q_outj_REP(j,t) = q_outj(j);
*p_outj_REP(j,t) = p_outj(j);
...
stop$((smax(A, abs(dev_XCREP(A))) lt (1/100)*1e-2) and (smax(C, abs(dev_PAREP(C))) lt (1/100)*1e-2))  = 1;
*stop$((smax(A, abs(dev_XCREP(A))) lt 1e-2) and (smax(C, abs(dev_PAREP(C))) lt 1e-2) and (smax(j, abs(dev_q_outj(j))) lt 1e-2) and (smax(j, abs(dev_p_outj(j))) lt 1e-2))  = 1;
...
Display S_new, C_new, q_outj, p_outj, ptc_j, dev_XCREP, dev_PAREP, counter;
