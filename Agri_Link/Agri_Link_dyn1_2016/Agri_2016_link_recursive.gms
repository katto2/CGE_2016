...
*operation=1;
SET
*$include "..\SAM\Agri\set_agri_recursive_link_2016_dyn.txt"
*$include "..\SAM\Agri\set_agri_recursive_nonlink_2016_dyn.txt"
$include ".\data\set_agri_recursive_link_2016_dyn.txt"
*$include ".\data\set_agri_recursive_nonlink_2016_dyn.txt"
...
$include ".\data\iocost_r_Agri_2016.csv"
...
$include ".\data\cost_Agri_2016.csv"
...
$include ".\data\b_sam_agri_model_g_pos_cons.csv"
...
$include ".\data\b_sam_agri_model_ng_pos_cons.csv"
...
$include ".\data\GHG_agri_model_process_cons.csv"
...
$include  ".\data\gtaxp_BAU.txt";
$include  ".\data\gtaxp_ctax_1.txt";
...
$include ".\data\lgrow.txt";
$include ".\data\Oldpopg.txt";
$include ".\data\TBg.txt";
...
BALCHK (iter,ACT)
GDPREP(*,iter)
SAMREP(iter,ACT,ACTPP)
GCREP(iter,GC,ACT,ACTP)
DGCREP(iter,GC,A)
VAREP(A,iter)
TGC(iter,GC)
...
TABLE  DATA(I,*,J) Data
$ondelim
$include ".\data\DataAll_160115.csv"
$offdelim;
...
S_new(J)=D0(J);
********************************************************************************
scalar          stop    Flag for signaling sufficient convergence       /0/,
                counter Counter for iteration numbers                   /0/;

parameter       dev_XCREP(A)     Deviation of XCREP values between subsequent solve,
                dev_PAREP(C)     Deviation of PAREP values between subsequent solve;

parameter       con_crit convergence criterion;
parameter       iterrep(t) iteration number for each t;
con_crit=1e-2;
Loop (t,
Loop (iter$(not stop),
*Loop (iter,
...
PCREP(A,iter)        =        PC.L(A)        ;
XCREP(A,iter)        =        XC.L(A)        ;
XMTREP(C,iter)        =        XMT.L(C)        ;
PMTREP(C,iter)        =        PMT.L(C)        ;
XDREP(C,iter)        =        XD.L(C)        ;
PDREP(C,iter)        =        PD.L(C)        ;
XAREP(C,iter)        =        XA.L(C)        ;
PAREP(C,iter)        =        PA.L(C)        ;
ESREP(C,iter)        =        ES.L(C)        ;
PETREP(C,iter)        =        PET.L(C)        ;
XPREP(C,iter)        =        XP.L(C)        ;
PPREP(C,iter)        =        PP.L(C)        ;
QNEGREP(C,A,iter)        =        QNEG.L(C,A)        ;
PNEGREP(C,A,iter)        =        PNEG.L(C,A)        ;
PVAEREP(A,iter)        =        PVAE.L(A)        ;
QVAEREP(A,iter)        =        QVAE.L(A)        ;
PAGREP(A,iter)=PAG.L(A);
XAGREP(A,iter)=XAG.L(A);
PHKTEREP(A,iter)=PHKTE.L(A);
HKTEREP(A,iter)=HKTE.L(A);
PEPREP(A,iter)=PEP.L(A);
XEPREP(A,iter)=XEP.L(A);
PFLREP(A,iter)=PFL.L(A);
XFLREP(A,iter)=XFL.L(A);
PLFLREP(A,iter)=PLFL.L(A);
XLFLREP(A,iter)=XLFL.L(A);
XAPREP(C,A,iter)        =        XAP.L(C,A)        ;
QINTGREP(GC,A,iter)        =        QINTG.L(GC,A)        ;
RREP(K,iter)        =        R.L(K)        ;
WREP(L,iter)        =        W.L(L)        ;
rentrep(iter)       =        rent.L;
LDREP(L,A,iter)        =        LD.L(L,A)        ;
KDREP(K,A,iter)        =        KD.L(K,A)        ;
ZDREP(A,iter)          =        ZD.L(A);
KsREP(K,iter)        =        Ks.L(K)        ;
LsREP(L,iter)        =        Ls.L(L)        ;
ZsREP(iter)          =        Zs.L;
QCEREP(C,A,iter)        =        QCE.L(C,A)        ;
QGEREP(GC,C,A,iter)        =        QGE.L(GC,C,A)        ;
EXRREP(iter)        =        EXR.L        ;
YGREP(iter)        =        YG.L        ;
TRREP(H,iter)        =        TR.L(H)        ;
XAFREP(FD,C,iter)        =        XAF.L(FD,C)        ;
SGREP(iter)        =        SG.L        ;
MPSREP(H,iter)        =        MPS.L(H)        ;
LYREP(H,iter)        =        LY.L(H)        ;
KYREP(H,iter)        =        KY.L(H);
EYREP(H,iter)        =        EY.L(H);
YHREP(H,iter)        =        YH.L(H)        ;
YDREP(H,iter)        =        YD.L(H)        ;
XACREP(C,H,iter)        =        XAC.L(C,H)        ;
SHREP(H,iter)        =        SH.L(H)        ;
TINSRREP(iter)        =        TINSR.L        ;
FSAVREP(iter)        =        FSAV.L        ;
IVADREP(iter)        =        IVAD.L        ;
WarlasREP(iter)        =        Warlas.L        ;
CREVREP(A,iter)        =        CREV.L(A)        ;
TCREVREP(iter)        =        TCREV.L        ;
ResinCREP(iter)        =        ResinC.L        ;
ResinIREP(iter)        =        ResinI.L        ;
CPIREP(iter)           = CPI.L;
gtaxrep(GC,iter)=gtax(GC);

SAMREP(iter,ENCN,A)=XAP.L(ENCN,A)*PA.L(ENCN);
SAMREP(iter,ENC,A)=QCE.L(ENC,A)*PA.L(ENC);
SAMREP(iter,K,A)=KD.L(K,A)*R.L(K);
SAMREP(iter,L,A)=LD.L(L,A)*W.L(L);
SAMREP(iter,'NRES',A)=alpha_nres(A)*XC.L(A)*PC.L(A);
SAMREP(iter,'Ptaxin',A)=ta_in(A)*XC.L(A)*PC.L(A);
SAMREP(iter,'Ptaxetc',A)=ta_ex(A)*XC.L(A)*PC.L(A);


SAMREP(iter,A,C)=theta(A,C)*XC.L(A)*PC.L(A);
SAMREP(iter,'Tarrif',C)=tm(C)*pwm(C)*EXR.L*XMT.L(C);
SAMREP(iter,'ROW',C)=pwm(C)*EXR.L*XMT.L(C);



SAMREP(iter,H,L)=W.L(L)*shr(L,H)*Ls.L(L);
SAMREP(iter,H,K)=R.L(K)*shr(K,H)*Ks.L(K)+sum(A$LinkA(A),shrpro(A,H)*rent.L*ZD.L(A));



SAMREP(iter,C,H)=PA.L(C)*XAC.L(C,H);
SAMREP(iter,'Ptaxin',H)=tc_in*sum(C,PA.L(C)*XAC.L(C,H));
SAMREP(iter,'Ytax',H)=TINSR.L*(YH.L(H)-sum(K,delta*shr(K,H)*KS.L(K)));
SAMREP(iter,'S-I',H)=SH.L(H);

SAMREP(iter,C,'Gov')=PA.L(C)*XAF.L('Gov',C);
SAMREP(iter,H,'Gov')=TR.L(H);
SAMREP(iter,'Ptaxin','Gov')=sum(C,tg_in*PA.L(C)*XAF.L('Gov',C));
SAMREP(iter,'S-I','Gov')=SG.L;
*
SAMREP(iter,'Household','NRES')=ResinC.L;
SAMREP(iter,'S-I','NRES')=ResinI.L;

SAMREP(iter,'Gov','Ptaxin')=sum(A,ta_in(A)*PC.L(A)*XC.L(A))+(tc_in)*sum((C,H)$(sam(C,H) ne 0),PA.L(C)*XAC.L(C,H))+tg_in*sum(C$(sam(C,'Gov')ne 0),PA.L(C)*XAF.L('Gov',C))+tiv_in*sum(C$(sam(C,'S-I') ne 0),PA.L(C)*XAF.L('S-I',C))+tm_in*sum(C$(sam('ROW',C) ne 0),PMT.L(C)*XMT.L(C));
SAMREP(iter,'Gov','Ptaxetc')=sum(A,ta_ex(A)*PC.L(A)*XC.L(A));
SAMREP(iter,'Gov','Tarrif')=sum(C$(sam('ROW',c)>0),tm(C)*EXR.L*pwm(C)*XMT.L(C));
SAMREP(iter,'GOV','Ytax')=sum(H, TINSR.L*(YH.L(H)-sum(K,delta*shr(K,H)*KS.L(K))));



SAMREP(iter,C,'S-I')=PA.L(C)*XAF.L('S-I',C);
SAMREP(iter,'Ptaxin','S-I')=sum(C,tiv_in*PA.L(C)*XAF.L('S-I',C));
SAMREP(iter,C,'ROW')=PET.L(C)*ES.L(C);
SAMREP(iter,'Ptaxin','ROW')=tm_in*sum(C$(sam('ROW',c)>0),PMT.L(C)*XMT.L(C));
SAMREP(iter,'S-I','ROW')=FSAV.L*EXR.L;

GDPREP('CON_P',iter)=sum((C,H), PA.L(C)*XAC.L(C,H));
GDPREP('CON_G',iter)=sum(C,PA.L(C)*XAF.L('Gov',C));
GDPREP('INV',iter)=sum(C,PA.L(C)*XAF.L('S-I',C));
GDPREP('EXP',iter)=sum(C$(sam(C,'ROW')>0),EXR.L*pwe(C)*ES.L(C));
GDPREP('IMP',iter)=-1*sum(C$(sam('ROW',c)>0),EXR.L*pwm(C)*XMT.L(C));
GDPREP('GDPMP1',iter)=sum(ACGDP1,GDPREP(ACGDP1,iter));
GDPREP('GDPFP',iter)=sum((K,A),R.L(K)*KD.L(K,A))+sum((L,A),W.L(L)*LD.L(L,A))+sum(H,EY.L(H));
GDPREP('NITAX',iter)=
...
GDPREP('NRES',iter)=sum(A$(sum(ACT,SAM(A,ACT))>0),alpha_nres(A)*PC.L(A)*XC.L(A));
GDPREP('GDPMP2',iter)=GDPREP('GDPFP',iter)+GDPREP('NITAX',iter)+GDPREP('NRES',iter);
GDPREP('GAP',iter)=GDPREP('GDPMP1',iter)-GDPREP('GDPMP2',iter);

VAREP(A,iter)=sum(K,R.L(K)*KD.L(K,A))+sum(L,W.L(L)*LD.L(L,A))+(ta_in(A)+ta_ex(A))*PC.L(A)*XC.L(A)+alpha_nres(A)*PC.L(A)*XC.L(A)+ZD.L(A)*rent.L;


GCREP(iter,GC,C,A)=QGE.L(GC,C,A);
GCREP(iter,GC,'process',A)=QINTG.L(GC,A);
GCREP(iter,GC,'Total',A)=sum(ACT,GCREP(iter,GC,ACT,A));

DGCREP(iter,GC,A)=GCREP(iter,GC,'Total',A);
TGC(iter,GC)=sum(A,GCREP(iter,GC,'Total',A));
BALCHK(iter,ACT)=sum(ACTPP,SAMREP(iter,ACT,ACTPP))-sum(ACTPP,SAMREP(iter,ACTPP,ACT));
...
dev_XCREP(A)$(ord(iter) gt 1)=100*(XCREP(A,iter)/XCREP(A,iter-1) - 1);
dev_PAREP(C)$(ord(iter) gt 1)=100*(PAREP(C,iter)/PAREP(C,iter-1) - 1);
display t,iter,XCREP,PAREP,dev_XCREP,dev_PAREP;

stop$((smax(A, abs(dev_XCREP(A))) lt con_crit) and (smax(C, abs(dev_PAREP(C))) lt con_crit))  = 1;
...
XPCon(C,t)        =        XP.L(C)        ;
PPCon(C,t)        =        PP.L(C)        ;
QNEGCon(C,A,t)        =        QNEG.L(C,A)        ;
PNEGCon(C,A,t)        =        PNEG.L(C,A)        ;
PVAECon(A,t)        =        PVAE.L(A)        ;
QVAECon(A,t)        =        QVAE.L(A)        ;
PAGCon(A,t)=PAG.L(A);
XAGCon(A,t)=XAG.L(A);
PHKTECon(A,t)=PHKTE.L(A);
HKTECon(A,t)=HKTE.L(A);
PEPCon(A,t)=PEP.L(A);
XEPCon(A,t)=XEP.L(A);
PFLCon(A,t)=PFL.L(A);
XFLCon(A,t)=XFL.L(A);
PLFLCon(A,t)=PLFL.L(A);
XLFLCon(A,t)=XLFL.L(A);
XAPCon(C,A,t)        =        XAP.L(C,A)        ;
QINTGCon(GC,A,t)        =        QINTG.L(GC,A)        ;
RCon(K,t)        =        R.L(K)        ;
WCon(L,t)        =        W.L(L)        ;
rentCon(t)       =        rent.L;
LDCon(L,A,t)        =        LD.L(L,A)        ;
KDCon(K,A,t)        =        KD.L(K,A)        ;
ZDCon(A,t)          =        ZD.L(A);
KsCon(K,t)        =        Ks.L(K)        ;
LsCon(L,t)        =        Ls.L(L)        ;
ZsCon(t)          =        Zs.L;
QCECon(C,A,t)        =        QCE.L(C,A)        ;
QGECon(GC,C,A,t)        =        QGE.L(GC,C,A)        ;
EXRCon(t)        =        EXR.L        ;
YGCon(t)        =        YG.L        ;
TRCon(H,t)        =        TR.L(H)        ;
XAFCon(FD,C,t)        =        XAF.L(FD,C)        ;
SGCon(t)        =        SG.L        ;
MPSCon(H,t)        =        MPS.L(H)        ;
LYCon(H,t)        =        LY.L(H)        ;
KYCon(H,t)        =        KY.L(H);
...
YHCon(H,t)        =        YH.L(H)        ;
YDCon(H,t)        =        YD.L(H)        ;
XACCon(C,H,t)        =        XAC.L(C,H)        ;
SHCon(H,t)        =        SH.L(H)        ;
TINSRCon(t)        =        TINSR.L        ;
FSAVCon(t)        =        FSAV.L        ;
IVADCon(t)        =        IVAD.L        ;
WarlasCon(t)        =        Warlas.L        ;
CREVCon(A,t)        =        CREV.L(A)        ;
TCREVCon(t)        =        TCREV.L        ;
ResinCCon(t)        =        ResinC.L        ;
ResinICon(t)        =        ResinI.L        ;
CPICon(t)           = CPI.L;
gtaxCon(GC,t)=gtax(GC);

SAMCon(t,ENCN,A)=XAP.L(ENCN,A)*PA.L(ENCN);
SAMCon(t,ENC,A)=QCE.L(ENC,A)*PA.L(ENC);
SAMCon(t,K,A)=KD.L(K,A)*R.L(K);
SAMCon(t,L,A)=LD.L(L,A)*W.L(L);
SAMCon(t,'NRES',A)=alpha_nres(A)*XC.L(A)*PC.L(A);
SAMCon(t,'Ptaxin',A)=ta_in(A)*XC.L(A)*PC.L(A);
SAMCon(t,'Ptaxetc',A)=ta_ex(A)*XC.L(A)*PC.L(A);


SAMCon(t,A,C)=theta(A,C)*XC.L(A)*PC.L(A);
SAMCon(t,'Tarrif',C)=tm(C)*pwm(C)*EXR.L*XMT.L(C);
SAMCon(t,'ROW',C)=pwm(C)*EXR.L*XMT.L(C);



SAMCon(t,H,L)=W.L(L)*shr(L,H)*Ls.L(L);
SAMCon(t,H,K)=R.L(K)*shr(K,H)*Ks.L(K)+sum(A$LinkA(A),shrpro(A,H)*rent.L*ZD.L(A));



SAMCon(t,C,H)=PA.L(C)*XAC.L(C,H);
SAMCon(t,'Ptaxin',H)=tc_in*sum(C,PA.L(C)*XAC.L(C,H));
SAMCon(t,'Ytax',H)=TINSR.L*(YH.L(H)-sum(K,delta*shr(K,H)*KS.L(K)));
SAMCon(t,'S-I',H)=SH.L(H);

SAMCon(t,C,'Gov')=PA.L(C)*XAF.L('Gov',C);
SAMCon(t,H,'Gov')=TR.L(H);
SAMCon(t,'Ptaxin','Gov')=sum(C,tg_in*PA.L(C)*XAF.L('Gov',C));
SAMCon(t,'S-I','Gov')=SG.L;
*
SAMCon(t,'Household','NRES')=ResinC.L;
SAMCon(t,'S-I','NRES')=ResinI.L;

SAMCon(t,'Gov','Ptaxin')=sum(A,ta_in(A)*PC.L(A)*XC.L(A))+(tc_in)*sum((C,H)$(sam(C,H) ne 0),PA.L(C)*XAC.L(C,H))+tg_in*sum(C$(sam(C,'Gov')ne 0),PA.L(C)*XAF.L('Gov',C))+tiv_in*sum(C$(sam(C,'S-I') ne 0),PA.L(C)*XAF.L('S-I',C))+tm_in*sum(C$(sam('ROW',C) ne 0),PMT.L(C)*XMT.L(C));
SAMCon(t,'Gov','Ptaxetc')=sum(A,ta_ex(A)*PC.L(A)*XC.L(A));
SAMCon(t,'Gov','Tarrif')=sum(C$(sam('ROW',c)>0),tm(C)*EXR.L*pwm(C)*XMT.L(C));
SAMCon(t,'GOV','Ytax')=sum(H, TINSR.L*(YH.L(H)-sum(K,delta*shr(K,H)*KS.L(K))));



SAMCon(t,C,'S-I')=PA.L(C)*XAF.L('S-I',C);
SAMCon(t,'Ptaxin','S-I')=sum(C,tiv_in*PA.L(C)*XAF.L('S-I',C));
SAMCon(t,C,'ROW')=PET.L(C)*ES.L(C);
SAMCon(t,'Ptaxin','ROW')=tm_in*sum(C$(sam('ROW',c)>0),PMT.L(C)*XMT.L(C));
SAMCon(t,'S-I','ROW')=FSAV.L*EXR.L;

GDPCon('CON_P',t)=sum((C,H), PA.L(C)*XAC.L(C,H));
GDPCon('CON_G',t)=sum(C,PA.L(C)*XAF.L('Gov',C));
GDPCon('INV',t)=sum(C,PA.L(C)*XAF.L('S-I',C));
GDPCon('EXP',t)=sum(C$(sam(C,'ROW')>0),EXR.L*pwe(C)*ES.L(C));
GDPCon('IMP',t)=-1*sum(C$(sam('ROW',c)>0),EXR.L*pwm(C)*XMT.L(C));
GDPCon('GDPMP1',t)=sum(ACGDP1,GDPCon(ACGDP1,t));
GDPCon('GDPFP',t)=sum((K,A),R.L(K)*KD.L(K,A))+sum((L,A),W.L(L)*LD.L(L,A))+sum(H,EY.L(H));
GDPCon('NITAX',t)=
sum(A$(sum(ACT,SAM(A,ACT))>0),(ta_in(A)+ta_ex(A))*PC.L(A)*XC.L(A))
+sum(C$(sam('ROW',c)>0),tm(C)*pwm(C)*XMT.L(C)*EXR.L)
+sum(C$(sam(C,'ROW')>0),te(C)*pwe(C)*ES.L(C)*EXR.L)
+TCREV.L
-sum(A,crevI*crevIw(A)*CREV.L(A));
GDPCon('NRES',t)=sum(A$(sum(ACT,SAM(A,ACT))>0),alpha_nres(A)*PC.L(A)*XC.L(A));
GDPCon('GDPMP2',t)=GDPCon('GDPFP',t)+GDPCon('NITAX',t)+GDPCon('NRES',t);
GDPCon('GAP',t)=GDPCon('GDPMP1',t)-GDPCon('GDPMP2',t);

VACon(A,t)=sum(K,R.L(K)*KD.L(K,A))+sum(L,W.L(L)*LD.L(L,A))+(ta_in(A)+ta_ex(A))*PC.L(A)*XC.L(A)+alpha_nres(A)*PC.L(A)*XC.L(A)+ZD.L(A)*rent.L;


GCCon(t,GC,C,A)=QGE.L(GC,C,A);
GCCon(t,GC,'process',A)=QINTG.L(GC,A);
GCCon(t,GC,'Total',A)=sum(ACT,GCCon(t,GC,ACT,A));

DGCCon(t,GC,A)=GCCon(t,GC,'Total',A);
TGCcon(t,GC)=sum(A,GCCon(t,GC,'Total',A));
BALCHKcon(t,ACT)=sum(ACTPP,SAMCon(t,ACT,ACTPP))-sum(ACTPP,SAMCon(t,ACTPP,ACT));
GDP1(t)=GDPcon('GDPMP1',t);

*
*PCcon(A,t)=PC.L(A);
**==== Law of Motion =====*
stop=0;
Ks.Fx('Capital')=Ks.L('Capital')*(1-delta)+(0.88)*sum(C,XAF.L('S-I',C));
...
*Display S_new, C_new, q_outj, p_outj, ptc_j,
display dev_XCREP, dev_PAREP, counter,con_crit;
display warlascon;
display GDPcon,GDP1;
display ygrow;
display TGCcon;
