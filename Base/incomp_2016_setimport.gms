...
$ONTEXT
- Oct 26. 2015
This model is for policy analysis
Data : 2010 IO table. Basic price data.
Base model: br_model_recursive.gms (completed in Sept.30)

Declaration is under way.

-Oct 27~Oct 28. 2015
Equations are addede for peculiar cases.
For export only/import only/no export/ no import cases
For composites that doesn't have full sub branches. (Equations done).

-Nov. 04: Incompatiblity analysis begins
1. Add four more years . t=0 is year 2010. t=24 is year 2035
2. update population growth prospect (new KOSIS data)
3. Adjuste lambdat to optain annual growth rate 2.8% in 2011-2035
4. Adjust lambda(A) to optain sector specific growth rate in 2011-2035
...
*operation=1;
SET
$include "set_standard_20160621_recursive.txt"
...
$Ondelim
$include b_sam_36_g_pos.csv
$Offdelim

table samng(ACT,ACTP) data in CSV format
$Ondelim
$include b_sam_36_ng_pos.csv
$Offdelim

table ghg(ACT,ACTP) data in CSV format
$Ondelim
$include GHG_model_p.csv
$Offdelim
...
elseif (scenario=2),
gtax_policy(GC,t)=gtaxp_ctax_2(GC,t);
CrevH=1;
CrevC=0;
CrevI=0;
CrevIw(A)=0;
...
elseif (scenario=3),
gtax_policy(GC,t)=gtaxp_ctax_3(GC,t);
CrevH=1;
CrevC=0;
CrevI=0;
CrevIw(A)=0;
...
gtax_policy(GC,t)=gtaxp_ctax_4(GC,t);
CrevH=1;
CrevC=0;
CrevI=0;
CrevIw(A)=0;
...
sigmaq('Cement-c')=        3        ;
sigmaq('Orgchem-c')=        3        ;
sigmaq('WoodPaper-c')=        1.9        ;
sigmaq('FiberLeather-c')=        1.9        ;
sigmaq('Mineral-c')=        1.9        ;
sigmaq('nonISmetal-c')=        3        ;
sigmaq('Machine-c')=        3        ;
sigmaq('Electro_e-c')=        3        ;
sigmaq('Electro_ne-c')=        3        ;
sigmaq('Electro_sig-c')=        3        ;
sigmaq('semicon-c')=        3        ;
sigmaq('Auto-c')=        3        ;
sigmaq('Ship-c')=        3        ;
sigmaq('Food-c')=        1.9        ;
sigmaq('MissManu-c')=        1.9        ;
sigmaq('Const-c')=        1.9        ;
sigmaq('Rail-c')=        1.9        ;
sigmaq('Road-c')=        1.9        ;
sigmaq('Air-c')=        1.9        ;
sigmaq('Marine-c')=        1.9        ;
sigmaq('MissTrans-c')=        1.9        ;
sigmaq('Housing-c')=        1.9        ;
sigmaq('Commercial-c')=        1.9        ;
sigmaq('Public-c')=        1.9        ;
sigmaq('Agri-c')=        3.29        ;
sigmaq('Waste-c')=        1.9        ;
...
PA0(C)=1;
*PET0('ELEC-C')=        1.242060742        ;
*PET0('GAS-c')=        0.957191718        ;
*PET0('HEAT-c')=        2.439723685        ;
*PET0('ROIL-c')=        1.043578169        ;
*PET0('CoalPro-c')=        0.484883817        ;
*PET0('Coal-c')=        0.484883817        ;
*PP0('ELEC-C')=        1.242060742        ;
*PP0('GAS-c')=        0.957191718        ;
*PP0('HEAT-c')=        2.439723685        ;
*PP0('ROIL-c')=        1.043578169        ;
*PP0('CoalPro-c')=        0.484883817        ;
*PP0('Coal-c')=        0.484883817        ;
*PC0('ELEC-a')=        1.242060742        ;
*PC0('GAS-a')=        0.957191718        ;
*PC0('HEAT-a')=        2.439723685        ;
*PC0('ROIL-a')=        1.043578169        ;
*PC0('CoalPro-a')=        0.484883817        ;
*PC0('Coal-a')=        0.484883817        ;
*PMT0('ELEC-C')=        1.242060742        ;
*PMT0('GAS-c')=        0.957191718        ;
*PMT0('HEAT-c')=        2.439723685        ;
*PMT0('ROIL-c')=        1.043578169        ;
*PMT0('CoalPro-c')=        0.484883817        ;
*PMT0('Coal-c')=        0.484883817        ;
*PD0('ELEC-C')=        1.242060742        ;
*PD0('GAS-c')=        0.957191718        ;
*PD0('HEAT-c')=        2.439723685        ;
*PD0('ROIL-c')=        1.043578169        ;
*PD0('CoalPro-c')=        0.484883817        ;
*PD0('Coal-c')=        0.484883817        ;
*PA0('ELEC-C')=        1.242060742        ;
*PA0('GAS-c')=        0.957191718        ;
*PA0('HEAT-c')=        2.439723685        ;
*PA0('ROIL-c')=        1.043578169        ;
*PA0('CoalPro-c')=        0.484883817        ;
*PA0('Coal-c')=        0.484883817        ;
EXR0=1;
...
KC_XLFL(C,A)$((LfuelA(A,C) and not (KC_maxind(C,A)=1)) and (Lfuel_single(A) ne 0))= ((sum(CP$(KC_maxind(CP,A)=1),PNEG0(CP,A))/PNEG0(C,A))**(1-sigmaaXLFL(A))*(SAM(C,A)/sum(CP$(KC_maxind(CP,A)=1),SAM(C,A)))*(AEEI(C,A)/sum(CP$(KC_maxind(CP,A)=1),AEEI(C,A)))**(1-sigmaaXLFL(A)))**(1/sigmaaXLFL(A));
...
TE_ROIL0
;
TE_ELEC0=sum(A,XAP0('Elec-c',A))+sum(H,XAC0('Elec-c',H))+sum(FD,XAF0(FD,'Elec-c'));
TE_Heat0=sum(A,XAP0('Heat-c',A))+sum(H,XAC0('Heat-c',H))+sum(FD,XAF0(FD,'Heat-c'));
TE_Gas0=sum(A,QCE0('Gas-c',A))+sum(H,XAC0('Gas-c',H))+sum(FD,XAF0(FD,'Gas-c'))-QCE0('Gas-c','Elec-a')-QCE0('Gas-c','Heat-a');
TE_Coal0=sum(A,QCE0('coal-c',A))+sum(H,XAC0('coal-c',H))+sum(FD,XAF0(FD,'coal-c'))-QCE0('coal-c','Elec-a')-QCE0('coal-c','Heat-a');
TE_Coalpro0=sum(A,QCE0('coalpro-c',A))+sum(H,XAC0('coalpro-c',H))+sum(FD,XAF0(FD,'coalpro-c'))-QCE0('coalpro-c','Elec-a')-QCE0('coalpro-c','Heat-a');
TE_ROIL0=sum(A,QCE0('Roil-c',A))+sum(H,XAC0('Roil-c',H))+sum(FD,XAF0(FD,'Roil-c'))-QCE0('Roil-c','Elec-a')-QCE0('Roil-c','Heat-a');
display TE_ELEC0,TE_Heat0,TE_Gas0,TE_Coal0,TE_Coalpro0,TE_ROIL0;
...
TE_ROIL(t)
;
TE_ELEC(t)=sum(A,XAPREP('Elec-c',A,t))+sum(H,XACREP('Elec-c',H,t))+sum(FD,XAFREP(FD,'Elec-c',t));
TE_Heat(t)=sum(A,XAPREP('Heat-c',A,t))+sum(H,XACREP('Heat-c',H,t))+sum(FD,XAFREP(FD,'Heat-c',t));
TE_Gas(t)=sum(A,QCEREP('Gas-c',A,t))+sum(H,XACREP('Gas-c',H,t))+sum(FD,XAFREP(FD,'Gas-c',t))-QCEREP('Gas-c','Elec-a',t)-QCEREP('Gas-c','Heat-a',t);
TE_Coal(t)=sum(A,QCEREP('coal-c',A,t))+sum(H,XACREP('coal-c',H,t))+sum(FD,XAFREP(FD,'coal-c',t))-QCEREP('coal-c','Elec-a',t)-QCEREP('coal-c','Heat-a',t);
TE_Coalpro(t)=sum(A,QCEREP('coalpro-c',A,t))+sum(H,XACREP('coalpro-c',H,t))+sum(FD,XAFREP(FD,'coalpro-c',t))-QCEREP('coalpro-c','Elec-a',t)-QCEREP('coalpro-c','Heat-a',t);
TE_ROIL(t)=sum(A,QCEREP('Roil-c',A,t))+sum(H,XACREP('Roil-c',H,t))+sum(FD,XAFREP(FD,'Roil-c',t))-QCEREP('Roil-c','Elec-a',t)-QCEREP('Roil-c','Heat-a',t);
...
display TE_ELEC,TE_Heat,TE_gas,TE_coal,TE_Coalpro,TE_ROIL;
