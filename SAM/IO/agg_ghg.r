...
GHG_36_0=aggregate(T_GHG_36_name[,2:5],list(T_GHG_36_name$sector_row),FUN=sum)
...
GHG_36_BR=GHG_36_Ind
GHG_36_BR$secname=rownames(GHG_36_BR)
GHG_36_BRname=merge(GHG_36_BR, sec_BR, by.x="secname", by.y="sector_name", all.x=T,sort=F)
GHG_row_BR=aggregate(GHG_36_BRname[,(2:(dim_GHG_36_Ind[2]+1))],list(GHG_36_BRname$BR_ind),FUN=sum)
...
GHG_BR_0=aggregate(T_GHG_BR_name[,2:4],list(T_GHG_BR_name$BR_ind),FUN=sum)
...
GHG_GR_0=aggregate(T_GHG_GR_name[,2:3],list(T_GHG_GR_name$Group_ind),FUN=sum)
...
write.csv(GHG_BR_p,file="GHG_BR_p.csv")
