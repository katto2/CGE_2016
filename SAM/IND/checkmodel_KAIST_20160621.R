...
#load data
KSIC.KAIST=read.csv(file="KCIS_KAIST_20160621.csv",header=T,as.is=T)
IO.KSIC=read.csv(file="2010_IO_KCIS.csv",header=T,as.is=T)
IO.KSIC.m=read.csv(file="2010_IO_KCIS_m.csv",header=T,as.is=T)
IO.model=read.csv("indcode_20160621.csv",header=T,as.is=T)
...
KSIC.m.core %>% arrange(CGE_code_mapp) %>% select(1:3) %>% write.csv(file="KSIC-model_20160621.mapp.csv")

IO.model.raw=read.csv("indcode_20160621.csv",header=T,as.is=T)
IO.model.sect=IO.model.raw[(1:384),(1:3)]
sec.dict=IO.model.raw[!is.na(IO.model.raw[,7]),7:8]
IO.sect=merge(IO.model.sect,sec.dict,by.x="sector_row",by.y="sector_ind",sort=F)
write.csv(IO.sect, file="IO-model_20160621.csv")
